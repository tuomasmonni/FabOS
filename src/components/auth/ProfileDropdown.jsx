...
export default function ProfileDropdown() {
  const { user, profile, signOut, openLoginModal, openProfilePage, isAuthenticated, isDemo, isAdmin, openAdminDashboard, openMyVersionsPage } = useAuth();
      ...
      {isOpen && (
        <div className={`absolute right-0 mt-2 w-56 rounded-xl overflow-hidden z-50 ${styles.dropdown}`}>
          ...
          <div className="py-1">
            ...
            {/* Admin-linkki - näkyy vain admin+ rooleille */}
            {isAdmin && isAdmin() && (
              <button
                onClick={() => {
                  setIsOpen(false);
                  if (openAdminDashboard) openAdminDashboard();
                }}
                className={`w-full px-4 py-2 text-left text-sm flex items-center gap-3 transition-colors ${styles.item}`}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              onClick={() => {
                setIsOpen(false);
                if (openMyVersionsPage) openMyVersionsPage();
              }}
              className={`w-full px-4 py-2 text-left text-sm flex items-center gap-3 transition-colors ${styles.item}`}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
