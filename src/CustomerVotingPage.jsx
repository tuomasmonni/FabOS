...
const CustomerVotingPage = ({ onBack }) => {
  ...
  const fetchVotes = useCallback(async () => {
    try {
      const response = await fetch(`${API_URL}/votes?category=customer`);
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    if (totalClicks >= 10 && !achievements.includes('10clicks')) {
      newAchievements.push({ id: '10clicks', title: 'Valittaja', emoji: '👆', description: '10 valitusta!' });
    }
    if (totalClicks >= 50 && !achievements.includes('50clicks')) {
      newAchievements.push({ id: '50clicks', title: 'Pro-valittaja', emoji: '🔥', description: '50 valitusta!' });
    }
    if (totalClicks >= 100 && !achievements.includes('100clicks')) {
      newAchievements.push({ id: '100clicks', title: 'Karen', emoji: '💅', description: '100 valitusta! Haluatko puhua esimiehen kanssa?' });
    }
    if (streak >= 10 && !achievements.includes('streak10')) {
      newAchievements.push({ id: 'streak10', title: 'Raivo!', emoji: '😤', description: '10 peräkkäistä valitusta!' });
    }
  ...
  const handleVote = async (topicId, optionId, e) => {
    ...
    // Lähetä palvelimelle
    try {
      const response = await fetch(`${API_URL}/votes?category=customer&topicId=${topicId}&optionId=${optionId}`, {
        method: 'POST'
      });
      if (response.ok) {
        const data = await response.json();
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 mx-auto mb-4 border-4 border-rose-500 border-t-transparent rounded-full animate-spin" />
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      {showAchievement && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-50 animate-bounce">
          <div className="bg-gradient-to-r from-rose-500 to-pink-500 text-white px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3">
      ...
      {/* Header */}
      <header className="border-b border-rose-500/30 px-6 py-4 bg-gradient-to-r from-rose-500/5 to-transparent">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-rose-400">Asiakaspalaute 2.0</h1>
            </div>
          </div>

          <div className="flex items-center gap-6">
            <div className="text-center">
              <div className="text-2xl font-black text-rose-400">{totalClicks}</div>
            ...
            {streak > 1 && (
              <div className="text-center animate-pulse">
                ...
                <div className="text-xs text-slate-500">Raivo!</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 text-center">
        <div className="inline-flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 rounded-full px-4 py-1.5 mb-6">
          <div className="relative">
            <div className="w-2 h-2 bg-rose-500 rounded-full" />
            <div className="absolute inset-0 w-2 h-2 bg-rose-500 rounded-full animate-ping" />
          </div>
          <span className="text-rose-400 text-sm font-medium">Live äänestys - Oikeat luvut!</span>
        </div>

        <h2 className="text-4xl md:text-5xl font-bold mb-4">
          Asiakas on aina
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-pink-400"> oikeassa</span>
        </h2>
        <p className="text-xl text-slate-400 max-w-2xl mx-auto">
          Vai onko? Äänestä ja päätä mihin suuntaan palvelua kehitetään. Lupaamme lukea tulokset... joskus.
        </p>
      </div>
      ...
      {/* Bottom bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur border-t border-rose-500/30 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="text-sm text-slate-400">
              Yhteensä <span className="text-rose-400 font-bold">
...
export default CustomerVotingPage;
